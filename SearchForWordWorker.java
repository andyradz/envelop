...
public class SearchForWordWorker extends SwingWorker<Integer, String> {
	...
	/**
	 * The directory under which the search occurs. All text files found under the given directory
	 * are searched.
	 */
	 ...
	 *        The word to search
	 * @param directory
	 *        the directory under which the search will occur. All text files found under the given
	 *        directory
	 *        are searched
	...
	protected Integer doInBackground() throws Exception {
		...
		/*
		 * List all text files under the given directory using the Apache IO library. This process
		 * cannot be
		 * interrupted (stopped through cancellation). That is why we are checking right after the
		 * process whether
		 * it was interrupted or not.
		 */
		publish("Listing all text files under the directory: " + directory);
		final List<File> textFiles =
				new ArrayList<>(FileUtils.listFiles(directory, new SuffixFileFilter(".txt"), TrueFileFilter.TRUE));
		...
		for (int i = 0, size = textFiles.size(); i < size; i++) {
			/*
			 * In order to respond to the cancellations, we need to check whether this thread (the
			 * worker thread)
			 * was interrupted or not. If the thread was interrupted, then we simply throw an
			...
			/*
			 * Read the file content into a string, and count the matches using the Apache common IO
			 * and Lang
