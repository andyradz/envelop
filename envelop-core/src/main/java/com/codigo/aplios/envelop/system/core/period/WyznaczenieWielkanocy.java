...
public class WyznaczenieWielkanocy {
	...
	public static void main(String[] args) {
		...
		// święto wielkiejnocy
		System.out.println("Wielkanoc:"
				+ DateTimeFormatter.ISO_DATE.format(CalendarEvents.EASTERDAY.queryFrom(now)));
		...
		System.out.println("Boże Ciało:"
				+ DateTimeFormatter.ISO_DATE.format(CalendarEvents.CORPUSCHRISTI.queryFrom(now)));

		// uroczystość miłosierdzia bożego tydzień po wielkanocy
		System.out.println("Miłosierdzie Boże:"
				+ DateTimeFormatter.ISO_DATE.format(CalendarEvents.CHARITABLENESS.queryFrom(now)));


		// uroczystość zielone świątki (pięcdzisiatnica)
		System.out.println("Zielone światki:"
				+ DateTimeFormatter.ISO_DATE.format(CalendarEvents.WHITSUNTIDE.queryFrom(now)));
...
enum CalendarEvents implements
...
enum EasterDayContext implements
	IEasterDayStrategy,
	TemporalQuery<LocalDate> {

		/**
		 * Klasa realizuje algorytm wyznaczania dnia wielkanocy metodą opublikowaną przez C.F.
		 * Gaussa.
		 * 
	...
	GAUSS_METHOD {
		...
		/**
		 * Klasa realizuje algorytm wyznaczania dnia wielkanocy metodą opublikowaną przez Jeana
		 * Meeusa.
		 * 
		 * @author Andrzej Radziszewski
		 *
