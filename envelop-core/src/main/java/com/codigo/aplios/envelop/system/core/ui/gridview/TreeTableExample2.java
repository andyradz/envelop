...
public class TreeTableExample2 {
	...
	protected JMenuBar createMenuBar() {
		...
		menuItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent ae) {
				...
				TreePath path = treeTable.getTree()
						.getSelectionPath();
	...
	class Reloader implements ActionListener, TreeExpansionListener {
		...
		protected void updateRow() {
			...
			if (reloadPath != null) {
				reloadRow = treeTable.getTree()
						.getRowForPath(reloadPath);
			}
		}
	}
	...
	class IndicatorRenderer extends DefaultTableCellRenderer {
		...
		public void paint(Graphics g) {
			...
			if (lastRow == reloadRow) {
				...
				if (reloadCounter < 5) {
					g.fillArc((width - diameter) / 2, (height - diameter) / 2, diameter, diameter, 90, -(reloadCounter
							* 90));
				} else {
					g.fillArc((width - diameter) / 2, (height - diameter) / 2, diameter, diameter, 90, (4
							- reloadCounter % 4) * 90);
				}
			} else {
				super.paint(g);
