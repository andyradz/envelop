...
public class TimeSpan {
	...
	public static void main(String[] args) {
		...
		LocalDate king = LocalDate.now().query(new NextMartinLutherKingDayQuery());
		...
		quantity = LocalDate.now().get(IsoFields.QUARTER_OF_YEAR);
		...
		quantity = Year.of(2016).length();
		quantity = Month.of(2).length(Year.of(2016).isLeap());

		quantity =
				ChronoField.DAY_OF_MONTH.getFrom(LocalDate.now().with(TemporalAdjusters.next(
						DayOfWeek.SUNDAY)));
