...
/**
 * Klasa realizuje mechanizm validacji numeru Pesel.
 * Implementuje domyślny model validacji standardu JavaBeans.
 * 
...
public class PeselValidator implements ConstraintValidator<PeselCheck, PeselIdentity> {
	...
	public boolean isValid(PeselIdentity value, ConstraintValidatorContext context) {
		...
		int checksum = +1
				* this.peselIdentity[0]
				+ 3
						* this.peselIdentity[1]
				+ 7
						* this.peselIdentity[2]
				+ 9
						* this.peselIdentity[3]
				+ 1
						* this.peselIdentity[4]
				+ 3
						* this.peselIdentity[5]
				+ 7
						* this.peselIdentity[6]
				+ 9
						* this.peselIdentity[7]
				+ 1
						* this.peselIdentity[8]
				+ 3
						* this.peselIdentity[9];
