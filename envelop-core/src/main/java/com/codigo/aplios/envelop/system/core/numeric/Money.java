package com.codigo.aplios.envelop.system.core.numeric;
 ...
 *
 * <P>
 * This class assumes <em>decimal currency</em>, without funky divisions
 ...
 * <P>
 * Operations can be performed on items having <em>different scale</em>.
 ...
 * 
 * <P>
 * <em>The {@link #init(CurrencyNew, RoundingMode)} method must be called at least 
...
public final class Money implements Comparable<Money>, Serializable {
	...
	public static void main(String[] args) {
		...
		Money money = new Money(BigDecimal.ZERO, Currency.getInstance(Locale.getDefault()), RoundingMode.HALF_UP);
		money =
				money.plus(new Money(BigDecimal.valueOf(16_500_789_250.09), Currency.getInstance(Locale
						.getDefault()), RoundingMode.HALF_UP));
		money =
				money.plus(new Money(BigDecimal.valueOf(4.24), Currency.getInstance(Locale
						.getDefault()), RoundingMode.HALF_UP));
	...
	public static final class MismatchedCurrencyException extends RuntimeException {
		private static final long serialVersionUID = -5551767756641368827L;
	 ...
	 *        decimals for the given {@link CurrencyNew}. It's possible to create a
	 ...
	 * 
	 * <P>
	 * Return <tt>true</tt> only if 'this' amount is greater than
	...
	public String toString() {
		...
		return df.format(fAmount.doubleValue()) + " " + fCurrency.getSymbol();
	}
	...
	private void validateState() {
		...
		if (fAmount.scale() > getNumDecimalsForCurrency()) {
			throw new IllegalArgumentException("Number of decimals is "
					+ fAmount.scale() + ", but currency only takes " + getNumDecimalsForCurrency() + " decimals.");
		}
	}
