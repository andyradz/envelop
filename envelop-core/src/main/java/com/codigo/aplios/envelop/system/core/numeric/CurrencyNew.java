...
public class CurrencyNew {
	...
	public static void main(String[] args) {
		...
		System.out.println(numberSigns[detectNumberSign((short) (ammount))]);
		...
		for (long index = 0; index < 1000005; index++) {
			...
			System.out.print(values[3][getHundreds((short) (ammount % 1E3))]);
			System.out.print(" ");
			System.out.print(values[2][getTens((short) (ammount % 1E3))]);
			System.out.print(" ");
			System.out.print(values[1][getTeensCount((short) (ammount % 1E3))]);
			System.out.print(values[0][getOnesCount((short) (ammount % 1E3))]);
			ammount++;
			System.out.println();
		}
	...
	public static final byte getOnesCount(short number) {
		...
		try {
			if (validNumber(number)) {
				return (((1 != (number / 10) % 10)) && (0 != (number % 10))) ? (byte) (number % 10) : 0;
			}
	...
	public static final byte getTeensCount(short number) {
		...
		try {
			if (validNumber(number))
				return ((0 != (number % 10)) && (1 == ((number / 10) % 10))) ? (byte) (number % 10) : 0;
		} catch (IllegalArgumentException ex) {
	 ...
	 * @param number
	 *        wartość liczby
	 * @return ilość nastek w liczbie, wartość liczbowa dodatnia gdy występują
	 *         jednostki, wartość 0 gdy brak jednostek, wartość -1 gdy liczba
	 *         nieprawidłowa
	 */
	public static final byte getTens(short number) {

		try {
			byte teensCount = getTeensCount(number);
			if (teensCount >= 0 && validNumber(number))
				return (0 == teensCount) ? (byte) ((number % 1E2) / 1E1) : 0;
		} catch (IllegalArgumentException ex) {
			// TODO zaloguj bład dod dziennika
			System.out.println(ex.getMessage());
		}
